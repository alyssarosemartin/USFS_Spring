...
RasterFolder <- "~/Documents/My Files/USA-NPN/Data/Analysis/R_default/USFS/BEST"
VectorFolder <- "~/Documents/My Files/USA-NPN/Data/Analysis/R_default/USFS/USFS_bound"
OutputFolder <- "~/Documents/My Files/USA-NPN/Data/Analysis/R_default/USFS"
...
plot(FLI_stack[[118:119]])  #check out a few
...
plot(FBI_stack[[119:122]])  #check out a few
...
write.csv(plit$FLI, file = 'USFS_FLI_values_geo_buff_1900-2021.csv')

#remove and remake plit
rm(plit, tab)
plit <- st_transform(pli, 4269) #code for NAD83
...
tab <-head(plit, n=5) #check this file
write.csv(plit$FBI, file = 'USFS_FBI_values_geo_buff_1900-2021.csv')

#Call back up the CSVs
setwd(OutputFolder)
FLY <- readr::read_csv("USFS_FLI_values_geo_buff_1900-2021.csv")
BLY <- readr::read_csv("USFS_FBI_values_geo_buff_1900-2021.csv")
...
FLI_forest <- t(as.matrix(FLY[c(2:121)]))
FBI_forest <- t(as.matrix(BLY[c(2:121)]))
...
write.csv(results, file = 'USFS_FLI_FBI_10_20_30_thru2021.csv')
...
setwd(VectorFolder)
st_write(results2map, 'usfs_results_thru2021.shp')
...
bins <- c(0, 5, 75, 95)  
cb <- c("red", "pink", "lightblue", "blue")
pal <- colorBin(palette = cb, bins = bins, domain = results2map$leaf_ave,  na.color = "transparent")


leaflet() %>% addTiles() %>%
  addPolygons(data = results2map, color = ~pal(results2map$leaf_ave), opacity = 1)
